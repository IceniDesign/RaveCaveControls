...
#define buttonPin_BLACK_A   2
int buttonState_BLACK_A = 0;        // variable for reading the pushbutton status
int Cycle_BLACK_A = 1;        // variable for reading the pushbutton status
#define buttonPin_BLUE_A   5
int buttonState_BLUE_A = 0;        // variable for reading the pushbutton status
int Cycle_BLUE_A = 1;        // variable for reading the pushbutton status
#define buttonPin_WHITE_A   7
int buttonState_WHITE_A = 0;        // variable for reading the pushbutton status
int Cycle_WHITE_A = 1;        // variable current Track Number
...
void loop()
{
  ...
  if (buttonState_BLACK_A == HIGH) {
    sendCommand(CMD_PLAY_WITHFOLDER, 0X0101);//play the song in folder 01, named 001xxxxxx.xxx
    delay(1000);
 }
    ...
    if (Cycle_WHITE_A == 1){ //if its 1 then play the first track and make the next one track 2
      sendCommand(CMD_PLAY_WITHFOLDER, 0X0601);
      Cycle_WHITE_A = 2;
      delay(1000);
      }
    else if (Cycle_WHITE_A == 2){
      sendCommand(CMD_PLAY_WITHFOLDER, 0X0602);
      Cycle_WHITE_A = 3;
      delay(1000);
      }
    else if (Cycle_WHITE_A == 3){
      sendCommand(CMD_PLAY_WITHFOLDER, 0X0603);
      Cycle_WHITE_A = 4;
      delay(1000);
      }
    else if (Cycle_WHITE_A == 4){
